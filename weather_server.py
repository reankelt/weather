...
"""
WRITE DETAILS ABOUT THIS FILE LIKE: IF IT USES MCP, FASTAPI AND OTHER FILES REFERENCED.
This file implements a local Flask web server that provides weather forecasts for US cities 
and states. 
It uses the National Weather Service (NWS) API to fetch forecast data and the Nominatim API 
for geocoding city/state names to coordinates. The server has an API endpoint at /api/forecast
that accepts a location query parameter and returns a JSON response with the forecast data. 
The main page served at / allows users to input a location and view the forecast in a 
user-friendly format. This file does not use MCP or FastAPI, but it can be tested using the 
test_mcp_client.py script by sending requests to the /api/forecast endpoint.
